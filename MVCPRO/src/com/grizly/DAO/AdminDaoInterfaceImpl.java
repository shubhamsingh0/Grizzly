...
public class AdminDaoInterfaceImpl implements AdminDaoInterface{
	...
	boolean flag=false;
	 ...
	 public int fetchuser(String username,String password) {
		 connection=DatabaseConnectionClass.getConnection();int  uidstatus=0;
	try{
			PreparedStatement statement = connection.prepareStatement("select USERID from login where USERNAME=? and PASSWORD=?");
			...
			if(resultset.next())
			{
				uidstatus=resultset.getInt("USERID");
			}
